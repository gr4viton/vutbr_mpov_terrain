...
%% TODO
% 1) vícekrát provést kmeans -> pro rùzné hodnoty nColors a ponechat tu
...
im = im_orig;
    ...
    imshow(im,[]); title(strcat('not preprocessed')); axis tight

%% preprocessing

im_prep = im_orig;
% im_prep = imfilter(
% im_prep_hsv = rgb2hsv(im_prep); 
% im_prep_hsv(:,:,2) = 1;         
% im_prep = hsv2rgb(im_prep_hsv);
for i=1:3
    im_prep(:,:,i) = histeq(im_prep(:,:,i), imhist(im_prep(:,:,i)));
end
